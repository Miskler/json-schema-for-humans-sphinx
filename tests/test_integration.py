...
'''
        ...
        assert find_schema_for_object('test_module.User.create', str(schema_dir)) is not None
        assert find_schema_for_object('test_module.User.update', str(schema_dir)) is not None
        assert find_schema_for_object('test_module.process_data', str(schema_dir)) is not None
        
        # Test that schemas are found correctly
        schema_result = find_schema_for_object('test_module.User.create', str(schema_dir))
    ...
    def test_config_integration(self, temp_dir):
        ...
        from jsoncrack_for_sphinx.config import RenderConfig, ContainerConfig
        ...
        delattr(mock_config_legacy, 'jsoncrack_default_options') if hasattr(mock_config_legacy, 'jsoncrack_default_options') else None
...
class TestSphinxIntegration:
    ...
    def test_rst_generation_integration(self, temp_dir):
        ...
        assert len(lines) > 5  # Should have title, separator, blank line, directive, content
